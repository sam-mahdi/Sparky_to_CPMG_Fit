...
def extract_parameters():
    ...
    with open('parameters.txt') as file:
        for lines in file:
            if lines.strip().split()[0] == '#assi':
                label=lines.split()
                for entries in label:
                    volume_search=re.search('^Volume',entries)
                    if volume_search is not None:
                        index_values.append(label.index(entries))
                continue
            line=lines.split()
            residue=line[0]
            volume_list=[]
            for values in index_values:
                volume_list.append(line[values])
            peak_height_list.append([' '.join(volume_list)])
            list_of_peaks.append(residue)
    return list_of_peaks,peak_height_list
