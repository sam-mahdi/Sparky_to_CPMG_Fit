...
def plot_cpmg_fit():
    ...
    with open(cval_file) as output_file:
        ...
        for lines in output_file:
            ...
            if counter == 1:
                ...
                if label not in label_list:
                    ...
                    axs[plot_counter_rows,plot_counter_columns].plot(cpfrq_list,experimental_value_list,color='red',label='Expreimental Data')
        ...
        total_variation=sum(total_variance_list)
        R2=1-(unexplained_variance/total_variation)
        axs[plot_counter_rows,plot_counter_columns].plot(cpfrq_list,experimental_value_list,color='red',label='Experimental Data')
        axs[plot_counter_rows,plot_counter_columns].plot(cpfrq_list,expected_value_list,'b--',label='Fitted Values')
        axs[plot_counter_rows,plot_counter_columns].set_title(f'{" ".join(display)} R2: {"{:.2f}".format(R2)}' )
        axs.[plot_counter_rows,plot_counter_columns].legend()
    fig.tight_layout()
